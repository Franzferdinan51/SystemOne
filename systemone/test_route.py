...
class StubEngine:
    ...
    def systemone(self, state, questions, batch_size=32):
        ...
        for q in questions:
            opts = q["options"]
            n = len(opts)
            # mimic the real GLiClass head on tier routing: near-uniform
            # probabilities, low confidence — it never drives the decision.
            raw = [1.0 + (0.1 if i == 0 else 0.0) for i in range(n)]
            total = sum(raw)
            probs = {o: r / total for o, r in zip(opts, raw)}
            answers[q["name"]] = {
                "type": "choice", "choice": opts[0],
                "probabilities": probs, "confidence": 0.45,
            }
        answers["_meta"] = {"model": "stub"}
        return answers


class _ConfidentStub:
    """Stub engine with configurable confident choice (for raise/escalation)."""

    def __init__(self, pick, confidence):
        self.pick = pick  # "first" or "last" option
        self.confidence = confidence
        self.model_name = "stub-confident"
    ...
    def systemone(self, state, questions, batch_size=32):
        ...
        for q in questions:
            opts = q["options"]
            n = len(opts)
            idx = 0 if self.pick == "first" else n - 1
            rest = (1.0 - self.confidence) / (n - 1) if n > 1 else 0.0
            probs = {o: (self.confidence if i == idx else rest)
                     for i, o in enumerate(opts)}
            answers[q["name"]] = {
                "type": "choice", "choice": opts[idx],
                "probabilities": probs, "confidence": self.confidence,
            }
        answers["_meta"] = {"model": "stub-confident"}
        return answers
...
ROUTING_TABLE = [
    ("summarize this product review in one sentence", "balanced", "economy"),
    ("what is the capital of France?", "balanced", "economy"),
    ("translate 'good morning' to Spanish", "balanced", "economy"),
    ("explain quantum entanglement to a 10-year-old", "balanced", "balanced"),
    ("draft a professional email about the delayed shipment", "balanced", "balanced"),
    ("help me plan my week", "balanced", "balanced"),
    ("write a quicksort in Python", "balanced", "balanced"),
    ("debug this intermittent deadlock in my multithreaded Rust service",
     "balanced", "heavy"),
    ...
    ("explain photosynthesis simply", "economy", "economy"),
